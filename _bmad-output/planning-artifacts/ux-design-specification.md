...
```mermaid
flowchart TD
    A[Entry: Recruiter opens homepage from resume, application, or outreach] --> B[Sees hero with identity, role framing, tags, and profile image]
    B --> C{Immediate clarity achieved?}
    C -- Yes --> D[Scrolls into supporting trust content]
    C -- No --> E[Uses nav, support copy, or visible cues to orient]
    E --> D
    D --> F{What proof do they want first?}
    F -- Resume --> G[Open resume summary or resume link]
    F -- Projects --> H[Open selected project previews]
    F -- General fit --> I[Scan background, skills, and positioning]
    G --> J{Resume reachable and current?}
    J -- Yes --> K[Trust and confidence increase]
    J -- No --> L[Trust failure / likely exit]
    H --> M[Review project evidence and summaries]
    I --> M
    M --> N{Enough signal to justify advancing?}
    N -- Yes --> O[Shortlist, revisit later, or contact]
    N -- No --> P[Defer or exit]
    K --> O
```
...
```mermaid
flowchart TD
    A[Entry: Evaluator opens homepage on mobile device] --> B[First screen loads with stable layout and clear identity]
    B --> C{Can they orient within moments?}
    C -- Yes --> D[Scrolls naturally]
    C -- No --> E[Checks visible trust signals or nav]
    E --> D
    D --> F{How much time do they have?}
    F -- Very little --> G[Uses quick path to resume or projects]
    F -- Some time --> H[Scrolls through key proof sections]
    G --> I{Target content easy to open and read on mobile?}
    H --> I
    I -- Yes --> J[Forms compressed trust judgment]
    I -- No --> K[Frustration / abandonment risk]
    J --> L{Site feels polished and reliable on mobile?}
    L -- Yes --> M[Moves Chris forward, saves link, or revisits later]
    L -- No --> N[Trust weakens]
    K --> N
```
...
```mermaid
flowchart TD
    A[Entry: Prospect lands on homepage from referral, message, or direct link] --> B[Assesses first-screen polish, confidence, and professionalism]
    B --> C{Does the site itself feel trustworthy and well-made?}
    C -- Yes --> D[Explores projects and background]
    C -- No --> E[Hesitates or exits]
    D --> F{What do they want next?}
    F -- Proof of work --> G[Open selected project details]
    F -- Personal credibility --> H[Review background or resume information]
    F -- Contact confidence --> I[Look for contact pathway]
    G --> J[Evaluates craftsmanship, clarity, and standards]
    H --> J
    J --> K{Does Chris appear capable of delivering quality work?}
    K -- Yes --> L[Reach out or keep Chris in active consideration]
    K -- No --> M[Exit without action]
    I --> N{Contact path clear and dependable?}
    N -- Yes --> L
    N -- No --> M
```
